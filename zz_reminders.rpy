default persistent._pronoun_mod_active_reminders = dict()


init 4 python in pronounMod_reminder_utils:

    import datetime

    INTERVAL_HOURLY_4 = datetime.timedelta(seconds=14400)
    INTERVAL_HOURLY_6 = datetime.timedelta(seconds=21600)
    INTERVAL_HOURLY_24 = datetime.timedelta(seconds=86400)

    LATENCY_HOURLY = datetime.timedelta(seconds=3600)


init 4 python in pronounMod_reminder:
    ...
    def addRecurringReminder(ev_label, delay, delta, latency):
        ...
        store.persistent._pronoun_mod_active_reminders[ev_label] = (
    ...
    def isReminderActive(ev_label):
        return ev_label in store.persistent._pronoun_mod_active_reminders

    def shouldTriggerReminder(ev_label):
        if not isReminderActive(ev_label):
            return False

        trigger, delta, latency = store.persistent._pronoun_mod_active_reminders[ev_label]
    ...
    def extendReminder(ev_label, keep_up=False):
        ...
        while True:
            ...
            if keep_up:
                if store.persistent._pronoun_mod_active_reminders[ev_label][0] <= now:
                    continue
                else:
                    break
            else:
    ...
    def stopReminder(ev_label):
        ...
        del store.persistent._pronoun_mod_active_reminders[ev_label]


init 7 python in pronounMod_reminder:
    ...
    def _handleMissedReminders():
        for ev_label in store.persistent._pronoun_mod_active_reminders.keys():
            trigger, delta, latency = store.persistent._pronoun_mod_active_reminders[ev_label]
